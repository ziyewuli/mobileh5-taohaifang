    ...
    <div class="swiper-bg ">
      <swiper v-if="swipeShow" class="school-swipe house-list" :options="swiperOption" ref="mySwiper1">
        <swiper-slide v-for="(list,index) in houseList" :key="list.id" class="toHere-bg">
          ...
          <div @click="routerHouseDetail(list.id)"  class="house-list-li ">
            <div class="top-img">
  ...
  export default{
    ...
    methods:{
      ...
      drawMap() {
        ...
        var latitude = self.houseList[0].latitude;
        var longitude = self.houseList[0].longitude;
        console.log("地图中心点坐标");
      ...
      routerList(){
        var self = this;
        this.$router.push({name:'房源比较',query:{id:self.id,status:self.status}})
      },
      /*筛选框的弹出逻辑开始*/
      toggle () {
        this.open = !this.open;
    ...
    mounted(){
      this.$nextTick(function () {
        ...
        if(self.$route.query){
          ...
          self.status=self.$route.query.status?self.$route.query.status:'';
          self.region=self.$route.query.region?self.$route.query.region:sessionStorage.getItem("code");
        }
        console.log(self.id);
          ...
          this.setupWebViewJavascriptBridge(function(bridge) {
            ...
            //alert("setupWebViewJavascriptBridge内部");
             bridge.registerHandler("getSearchData",function(data,responseCallback){
                responseCallback({id:self.id,status:self.status,region:self.region});
             });
          })
        }
        //查询周边房源
        self.$http.get('/residence/compare.json', {'params':{'id':self.id,status:self.status,region:self.region}}, {emulateJSON: true})
          .then(function (response) {
            ...
            if(response.body.data!==null){
              ...
              self.houseList = self.status==1?self.compareData.saleList:self.compareData.soldList;
              self.showHouseList();
            }else{
              self.drawMap();
            }
