<template>
  ...
  <div class="index" >
    ...
    <div class="top-container" ref="viewBox">
      <div class="search-box clearfix" v-show="scroll>=71">
        <div class="city float-l" @click="open"><i class="icon iconfont icon-icon_shouye_dingweidian"></i>{{cityName}}</div>
        <div class ="map float-r" @click="routerMap()"><i class="icon iconfont icon-icon-ditu"></i>地图</div>
        <div class="input float-r" @click="showHouse"><i class="icon iconfont icon-icon_shouye_fangdajing"></i>搜索房源</div>
      </div>
      <div class="search-box2 clearfix">
        <div class="city float-l" @click="open"><i class="icon iconfont icon-icon_shouye_dingweidian"></i>{{cityName}}</div>
        <div class ="map float-r" @click="routerMap()"><i class="icon iconfont icon-icon-ditu"></i>地图</div>
        <div class="input float-l" @click="showHouse"><i class="icon iconfont icon-icon_shouye_fangdajing"></i>搜索房源</div>
      </div>
      <div class="nav-container-bg">
        <div class="nav-container">
          <ul class="clearfix">
            <li v-for="item in menu" :key="item.sort" @click="menuRouter(item.type)">
              <div class="menuIcon" :class="item.type"></div>
              <!--<i class="icon iconfont icon-icon-ershoufang" :class="menuIcon[item.type]"></i>-->
              <p>{{menuText[item.type]}}</p>
            </li>
          </ul>
        </div>
        <div class="notice-container clearMargin clearfix">
          <img class="float-l" src="../assets/img/icon-notice.png">
          <div class="scroll-wrap float-l">
            <ul class="scroll-content" :style="{ top }">
              <li v-for="banner in banners2" :key="banner.id">
                <div @click="routerNews(banner.id)">
                  <nobr>{{banner.title}}</nobr>
                </div>
              </li >
            </ul>
          </div>
      ...
      <div class="banner-bg clearfix" v-if="banners.length">
          <div class="banner-l float-l">
            <img :src="bannerImg+banners[0].image" alt="" @click="toBanner(banners[0])">
          </div>
          <div class="banner-r float-l">
            <div class="banner-top">
              <img :src="bannerImg+banners[1].image" alt="" @click="toBanner(banners[1])">
            </div>
            <div class="banner-bottom">
              <img :src="bannerImg+banners[2].image" alt="" @click="toBanner(banners[2])">
            </div>
          </div>
      </div>
      <!--<mt-swipe :auto="3000">
        <mt-swipe-item  v-for="banner in banners" :key="banner.id"><img :src="bannerImg+banner.image" alt="" @click="toBanner(banner)"></mt-swipe-item>
      </mt-swipe>-->
    </div>
    <div class="container container-guess" v-if="guessList&&guessList.length>0">
      <div class="container-title">
        <span >猜你喜欢</span>
        <div class ="more" @click="guessDataNext" v-if="totalPage>1">
      ...
      <div class="list-bg">
        <ul  class="house-list" >
          <li v-for="list in guessList" @click="routerDetail(list.id)">
            <div class="top-img">
    ...
    <div class="container">
      <div class="container-title">
        <span >海房优选</span>
        <div class ="more" @click="routerList1" >
          <span>更多</span>
      ...
      <div class="list-bg">
        ...
        <div v-if="showClear" @click="clear()">清除缓存</div>
      </div>
    </div>
    <!--<div class="container">
      <div class="banner3">
        <div class="big-left"><img :src="bannerImg+centerBanner[0]" alt=""></div>
        <div class="small-right">
          <div><img :src="bannerImg+centerBanner[1]" alt=""></div>
          <div><img :src="bannerImg+centerBanner[2]" alt=""></div>
        </div>
      </div>
      <div class="swipe-box-box">
        <p @click="routerList('message')">热门资讯 <i class="icon iconfont icon-icon_shouye_youjiantou"></i></p>
        <swiper class="index-t" :options="swiperOption" ref="mySwiper1">
          <swiper-slide v-for="banner in banners2" :key="banner.id">
            <router-link :to="{name:'资讯详情',params:{id:banner.id}}">
              <img :src="imgUrl+banner.imgUrl" alt="">
              <p v-text="banner.title"></p>
            </router-link>
          </swiper-slide>
        </swiper>
      </div>
      <div class="swipe-box-box1">
        <p><router-link to="ask">热门问答<i class="icon iconfont icon-icon_shouye_youjiantou"></i></router-link></p>
        <swiper class="index-ask" :options="swiperOption" ref="mySwiper2">
          <swiper-slide v-for="(banner, key, index) in banners3" :key="banner.id">
            <div style="height:100%;" @click="routerAskDetail(banner.id)">
              <img v-if="(key+1)%3==1" src="../assets/img/bag1.png" alt="">
              <img v-if="(key+1)%3==2" src="../assets/img/bag2.png" alt="">
              <img v-if="(key+1)%3==0" src="../assets/img/bag3.png" alt="">
              <p v-text="banner.title"></p>
            </div>
          </swiper-slide>
        </swiper>
      ...
      <div class="cuish">
        <p @click="routerList('house')" >房源推荐<i class="icon iconfont icon-icon_shouye_youjiantou"></i></p>
        <swiper class="cuish-list" :options="swiperOption" ref="mySwiper3">
          <swiper-slide v-for="list in data" :key="list.id">
            <router-link :to="{name:'房源详情',params:{id:list.mslno}}">
              <div class="left-img">
                <p class="room-num">{{list.beds}}卧·{{list.baths}}卫·{{list.interiorfloorspace}}sqft</p>
                <div class="small-tip"><span v-text="list.marketTime"></span></div>
              ...
              <div class="center-content">
                <div v-if="list.buildingType=='Apartment'">{{list.region=='YTO'?'多伦多':'温哥华'}}-公寓</div>
                <div v-if="list.buildingType=='House'">{{list.region=='YTO'?'多伦多':'温哥华'}}-独立屋</div>
                <div v-if="list.buildingType=='Row/Townhouse'">{{list.region=='YTO'?'多伦多':'温哥华'}}-排屋</div>
                <div class="right-cash">
                  <span class="creat-time" v-text="list.builtIn=='0'?'建造年份未知':list.buildIn"></span>
                  <div class="money">$<span>{{(list.price/10000).toFixed(2)}}万</span></div>
                </div>
              </div>
            </router-link>
          </swiper-slide>
    ...
    <mu-dialog  dialogClass="choiceCityDialog" :open="rightPopup" @close="close()" scrollable>
      <div class="mu-appbar mu-paper-1">
        <div @click="closeQuery" class="left"><i class="icon iconfont icon-back"></i></div>
        <div class="mu-appbar-title"><span>选择城市</span></div>
        <div @click="close()" class="right"></div>
      </div>
      <div class="all-region">
        <!--<p>当前定位城市</p>
        <div class="clearfix">
          <div @click="showToast('该城市没有数据，请切换');" class="cityName"><i class="icon iconfont icon-icon_xuanzequyu_dingwei"></i> {{locationCity}}</div>
        </div>-->
        <p>加拿大</p>
        <div class="clearfix">
          <div @click="cityNameChange('温哥华')" class="cityName">温哥华</div>
          <div @click="cityNameChange('多伦多')" class="cityName">多伦多</div>
        </div>
        <p>美国</p>
        <div class="clearfix">
          <div  class="cityName" @click="cityNameChange('纽约');">纽约</div>
          <div  class="cityName" @click="cityNameChange('旧金山');">旧金山</div>
          <div  class="cityName" @click="cityNameChange('洛杉矶');">洛杉矶</div>
          <div  class="cityName" @click="cityNameChange('西雅图');">西雅图</div>
        </div>
        <p>澳大利亚</p>
        <div class="clearfix">
          <div  class="cityName" @click="showToast('该城市没有数据，请切换');">悉尼<span class="c-9B9B9B">（敬请期待）</span></div>
          <div  class="cityName" @click="showToast('该城市没有数据，请切换');">墨尔本<span class="c-9B9B9B">（敬请期待）</span></div>
        </div>
      </div>
    </mu-dialog>
    <mu-dialog dialogClass="screen-dialog  cityLog3" :open="dialog" @close="closeDiage" scrollable>
      <div class="mu-appbar mu-paper-1">
    ...
    <mu-dialog :open="cityChoice" title="" dialogClass="cityChoiceDialog">
    <!--<mu-dialog :open="cityChoice" title="" dialogClass="cityChoiceDialog">-->
      <p class="cityLabel">您当前定位的城市:</p>
      <p class="cityName">{{locationCity}}</p>
      <div class="choiceCity-btn" @click="open">选择其他城市</div>
    </mu-dialog>
    <div class="mu-paper mu-paper-round mu-paper-1" style="display: block;position: fixed;bottom:0;z-index: 2147647;">
      <div tabindex="0" class="mu-bottom-nav"
        ...
        <div class="mu-bottom-nav-shift-wrapper">
          <button @click="routerNav('首页')" type="button" tabindex="0" class="mu-buttom-item mu-bottom-item-active"
                  style="user-select: none; outline: none; cursor: pointer; -webkit-appearance: none;">
            <div class="mu-buttom-item-wrapper">
              <div class="mu-ripple-wrapper"></div>
              <i class="mu-bottom-item-icon mu-icon material-icons icon iconfont icon-icon_shouye_yixuan"> </i> <span
...
<style lang="less" rel="stylesheet/less" type="text/less">
  ...
  div.cityChoiceDialog{
    width:265/@base;
    height:330/@base;
    background-image: url('../assets/img/cityDialog.png');
    ...
    .top-container{
      ...
      .nav-container{
        ...
        li{
          ...
          .menuIcon.secondHouse{
            background-image: url('../assets/img/secondHouse.png');
          }
          .menuIcon.schoolHouse{
            background-image: url('../assets/img/schoolHouse.png');
          }
          .menuIcon.consult{
            background-image: url('../assets/img/consult.png');
          }
          .menuIcon.knowledge{
            background-image: url('../assets/img/knowledge.png');
          }
          .menuIcon.rentHouse{
            background-image: url('../assets/img/rentHouse.png');
          }
          .menuIcon.saleHouse{
            background-image: url('../assets/img/saleHouse.png');
          }
          .menuIcon.findAgent{
            background-image: url('../assets/img/findAgent.png');
          }
          .menuIcon.trend{
...
<script>
  ...
  export default{
    data(){
        return {
          cityMap:{
              "多伦多":{code:'ca0101',cityName:'多伦多',country:'ca',region:'YTO',detailName:'房源详情'},
              "温哥华":{code:'ca0201',cityName:'温哥华',country:'ca',region:'YVR',detailName:'房源详情'},
              "纽约":{code:'us0101',cityName:'纽约',country:'us',region:'us0101',detailName:'美国房源详情'},
              "旧金山":{code:'us0201',cityName:'旧金山',country:'us',region:'us0201',detailName:'美国房源详情'},
              "洛杉矶":{code:'us0202',cityName:'洛杉矶',country:'us',region:'us0202',detailName:'美国房源详情'},
              "西雅图":{code:'us0301',cityName:'西雅图',country:'us',region:'us0301',detailName:'美国房源详情'}
              },
          guessList:null,
          showClear:false,
          menu:[],
          cadRate:null,
          usdRate:null,
          menuIcon:{'secondHouse':'secondHouse','rentHouse':'icon-icon-zufang','schoolHouse':'icon-icon-xuequzhaofang','consult':'icon-icon-zixun','saleHouse':'icon-icon-maifang','findAgent':'icon-icon-zhaojingjiren','knowledge':'icon-icon-goufangbaike','trend':'icon-icon-loushizoushi'},
          menuText:{'secondHouse':'二手房','rentHouse':'租房','schoolHouse':'学区找房','consult':'资讯','saleHouse':'我是房主','findAgent':'找经纪人','knowledge':'购房百科','trend':'楼市走势'},
          inData:{
            chineselng:'',
      ...
      this.$nextTick(function () {
        ...
        self.$http.get('/cfg/config.json', {}, {emulateJSON: true})
          .then(function (response) {
            ...
            self.cadRate=response.data.data.cadRate;
        ...
        self.cityName= sessionStorage.getItem('cityName')?sessionStorage.getItem('cityName'):'温哥华';
        var cityObj = self.cityMap[self.cityName];
        ...
        self.showClear = rxUtils.variable.isTest;

        if(window.location.href.indexOf('code')>-1&&window.location.href.indexOf('region')==-1){
          let code=window.location.href.split('#')[0].split('?')[1].split('&')[0].split('=')[1];
          if(sessionStorage.getItem('wxCode')!==undefined&&sessionStorage.getItem('wxCode')!==code){
            sessionStorage.setItem('wxCode',code);
            self.$http.get('/wx/auth.json?code='+sessionStorage.getItem('wxCode')+'&authType=3', {}, {emulateJSON: true})
              .then(function (response) {
        ...
        this.hostPhoto=rxUtils.hostPhoto//头像
        this.hostRentHouse=rxUtils.hostRentHouse,//租房
        this.hostSchool=rxUtils.hostSchool,//学校
        this.hostHouse=rxUtils.hostHouse,//房子
        this.hostAsk=rxUtils.hostAsk,//问答
        this.bannerImg=rxUtils.banner//轮播
        this.imgUrl=rxUtils.encImg;
        //海房优选
        ...
        //微头条
        self.$http.get('/info/indexList.json?region='+this.region, {}, {emulateJSON: true})
          .then(function (response) {
            self.banners2=response.data.data;
          }).then(function (error) {
        })
        //轮播banner
        self.$http.post('/banner/listBanners.json?position=2&country='+self.country, {}, {emulateJSON: true})
          .then(function (response) {
        ...
        /*self.$http.post('/banner/listBanners.json?position=2', {}, {emulateJSON: true})
          .then(function (response) {
            self.centerBanner[0]=response.data.data[0].image
            self.centerBanner[1]=response.data.data[1].image
            self.centerBanner[2]=response.data.data[2].image
          }).then(function (error) {
        })//固定banner*/
        /*
        * ca0101  多伦多地区
        ...
        * */
        ...
        setTimeout(function(){
          self.sliderOption.measureNum=[0,5000]
          self.sliderOption1.measureNum=[0,1000]
        },500);
        setInterval(_ => {
          if(self.activeIndex < self.banners2.length-1) {
            self.activeIndex += 1;
          } else {
            self.activeIndex = 0;
          }
    ...
    methods:{
      guessYouLike(){
        ...
        if(authentication){
          self.$http.get('/statistics/residenceGuessParam.json?region='+self.region, {}, {emulateJSON: true}).then(function (response) {
            console.log('获取猜你喜欢条件成功');
            if(response.data.data){
              self.guessData=response.body.data;
              self.guessData.pageSize=3;
              self.guessData.currentPage=1;
              console.log(self.guessData);
              self.search(self.guessData);
            }else{
              self.guessList=[];
            }
          })
      ...
      guessDataNext(){
        ...
        console.log(self.guessData.currentPage,self.totalPage);
        self.guessData.currentPage = self.guessData.currentPage<self.totalPage?self.guessData.currentPage+1:1;
        self.search(self.guessData);
      },
      search(guessData){
        let self=this;
        self.$http.post('/residence/search.json', guessData, {emulateJSON: true})
          .then(function (response) {
            self.guessList=response.body.data;
            self.totalResults=response.body.totalResults;
            self.totalPage=response.body.totalPage;
            console.log(self.guessList);
            console.log("猜你喜欢结果");
          })
      },
      routerNews(id){
        _czc.push(['_trackEvent', '首页', 'HomeNewsBanner','微头条id='+id]);
        this.$router.push({name:'资讯详情',params:{id:id}})
      },
      toBanner(banner){
        if(banner.urlType==1&&banner.url!='／'&&banner.url!='/'){
          //绝对地址，直接打开
          if(banner.url.indexOf('#/buyProUS')>-1){
            this.$router.push({name:'买房过程美国'})
          }else if(banner.url.indexOf('#/loanProUS')>-1){
            this.$router.push({name:'贷款流程美国'})
          }else if(banner.url.indexOf('#/loanPro')>-1){
            this.$router.push({name:'贷款流程'})
          }else if(banner.url.indexOf('#/buyPro')>-1){
            this.$router.push({name:'买房过程'})
          }else if(banner.url.indexOf('#/bannerShare')>-1){
            this.$router.push({name:'分享专题'})
          }else{
        ...
        _czc.push(['_trackEvent', '首页', 'HomeImageBanner','轮播图id='+banner.id]);
      },
      clear(){
        localStorage.clear();
        alert("清除缓存成功");
      ...
      //展示筛选页
      showHouse(){
        _czc.push(['_trackEvent', '首页','HomeSearchHouse', '搜索框']);
        this.$router.push({name:'房源筛选'})
      },
      //选择城市
      ...
      searchByMls(){
        ...
        self.$http.get('/residence/getDetail.json?mls='+self.mlsNum, {}, {emulateJSON: true}).then(function (response) {
          this.dialog =false;
          if(response.body.data!==null){
            this.$router.push({name:sessionStorage.getItem('detailName')?sessionStorage.getItem('detailName'):'房源详情',params:{id:self.mlsNum},query:{region:sessionStorage.getItem('code')}})
          }else{
      ...
      menuRouter(type){
          let self = this;
          if(type=='secondHouse'){
            _czc.push(['_trackEvent', '首页', 'SecondaryHouse','二手房']);
            ...
            _czc.push(['_trackEvent', '首页', 'SchoolFindHouse','学区找房']);
            ...
            _czc.push(['_trackEvent', '首页', 'News','资讯']);
            ...
            _czc.push(['_trackEvent', '首页', 'HomeWiki','我是房主']);
            this.$router.push({name:'我是房主',query:{
              region:this.region,
            ...
            _czc.push(['_trackEvent', '首页', 'HomeWiki','购房百科']);
          ...
          function developing() {
            self.$router.push({name:'开发中',query:{title:self.menuText[type]}})
          }
      },
      routerNav(name){
          if(name=='首页'){
      ...
      routerList1(type){
        _czc.push(['_trackEvent', '首页', 'HomeMoreButton','首页更多按钮']);
        this.$router.push({name:'找房列表',query:{
          region:this.region,
          city:this.routerData.city,
          code:this.code
      ...
      routerDetail(id){
        _czc.push(['_trackEvent', '首页', 'HomeHouseClick','海房优选mslno='+id]);
...
//      },//清空历史搜索数据
      cityNameChange(name){
        let self=this;
        var cityObj = self.cityMap[name];
        self.cityName=cityObj.cityName;
        self.code=cityObj.code;
        self.region=cityObj.region;
        self.country=cityObj.country;
        self.detailName=cityObj.detailName;
        _czc.push(['_trackEvent', '首页','Choose', '选择'+name]);
        sessionStorage.setItem('cityName',self.cityName)
        sessionStorage.setItem('region',self.region)
        sessionStorage.setItem('code',self.code)
        sessionStorage.setItem('country',self.country)
        ...
        //微头条
        self.$http.get('/info/indexList.json?region='+self.region, {}, {emulateJSON: true})
          .then(function (response) {
            self.banners2=response.data.data;
          }).then(function (error) {
        })
      ...
      cityNameChange1(name){
        ...
        if(name=='多伦多'){
          ...
          _czc.push(['_trackEvent', '首页','ChooseYTO', '选择多伦多']);
        }else {
          this.region='YVR';
          this.code='ca0201'
          localStorage.setItem('location','温哥华')
          _czc.push(['_trackEvent', '首页','ChooseYVR', '选择温哥华']);
        }
        sessionStorage.setItem('region',self.region)
        sessionStorage.setItem('code',self.code)
        self.$http.get('/residence/listRecommend.json?pageSize=10&currentPage=1&region='+self.region, {}, {emulateJSON: true})
      ...
      open(type) {
        _czc.push(['_trackEvent', '首页','ChooseCity', '选择城市']);
        this.activeTab=type;
          ...
          geolocation.getCurrentPosition(function (r) {
            if (this.getStatus() == BMAP_STATUS_SUCCESS) {
              ...
              geoc.getLocation(pt, function (rs) {
                ...
                localStorage.setItem('locationCity',self.locationCity);
                return city;
              })
            }
          })
...
//                this.routerData.beds=5;
      ...
      routerMap(){
        _czc.push(['_trackEvent', '首页', 'MapFindHouse','地图找房']);
